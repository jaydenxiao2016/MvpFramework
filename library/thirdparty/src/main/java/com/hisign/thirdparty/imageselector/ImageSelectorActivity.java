...
import com.hisign.thirdparty.imageselector.utils.Utils;
...
public class ImageSelectorActivity extends FragmentActivity implements ImageSelectorFragment.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        submitButton.setVisibility(imageConfig.isMultiSelect() ? View.VISIBLE : View.GONE);
    ...
    @Override
    public void onImageSelected(List<String> resultList) {
        pathList.clear();
        pathList.addAll(resultList);
        if (pathList.size() > -1 && imageConfig.isMultiSelect() ) {
            submitButton.setText((getResources().getText(R.string.finish)) + "(" + pathList.size() + "/" + imageConfig.getMaxSize() + ")");
            if (pathList.size() > 0) {
                submitButton.setEnabled(true);
            } else {
